...
export default function RegistryPreviewRecord({ record, onBack }: { record: Record<string, unknown>; onBack: () => void }) {
  ...
  return <main className="main oracle-detail registry-record">
    ...
    <p className="eyebrow">Source-reviewed registry · signed dataset</p>
    ...
    <p className="registry-review-note">This record is part of the signed registry dataset. Issuer documents and dated on-chain mint or contract checks establish the source trail, not backing, redemption performance or executable liquidity. The attestation covers the published bytes, not the accuracy of issuer claims.</p>
